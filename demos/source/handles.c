...
static DWORD customCheckState;
static long WINAPI checkProc(HWND h,DWORD msg,DWORD wp,long lp){if(msg==0xf1){customCheckState=wp;return 0;}if(msg==0xf0)return customCheckState;return DefWindowProcA(h,msg,wp,lp);}
...
void mainCRTStartup(void){
  ...
  CHECK(SetClassLongW(encodingWindow,-16,0)==(DWORD)GetModuleHandleA(NULL));CHECK(!GetClassLongA(encodingWindow,-16));CHECK(!SetClassLongA(encodingWindow,-16,(long)GetModuleHandleA(NULL)));
  CHECK(SetClassLongW(encodingWindow,-18,12)==8);CHECK(GetClassLongA(encodingWindow,-18)==12);CHECK(SetClassLongA(encodingWindow,-18,8)==12);SetLastError(1234);CHECK(!SetClassLongA(encodingWindow,-20,8));CHECK(GetLastError()==87);
  CHECK(GetClassWord(encodingWindow,-32)==encodingAtomA);CHECK(!SetClassWord(encodingWindow,6,0xabcd));CHECK(GetClassWord(encodingWindow,6)==0xabcd);CHECK(SetClassWord(encodingWindow,6,0)==0xabcd);SetLastError(1234);CHECK(!GetClassWord(encodingWindow,7));CHECK(GetLastError()==1413);
  CHECK(!SetWindowWord(encodingWindow,6,0xabcd));CHECK(GetWindowWord(encodingWindow,6)==0xabcd);CHECK(SetWindowWord(encodingWindow,6,0)==0xabcd);
  {static WORD classNameBuffer[8];CHECK(GetClassNameW(encodingWindow,classNameBuffer,8)==2);CHECK(classNameBuffer[0]==65&&classNameBuffer[1]==233);classNameBuffer[0]=123;CHECK(!GetClassNameW(encodingWindow,classNameBuffer,1));CHECK(GetLastError()==122);CHECK(classNameBuffer[0]==123);}
  {HWND control=CreateWindowExA(0,"STATIC","lookup",0x40000000,0,0,20,20,encodingWindow,(HANDLE)77,0,0);CHECK(control);CHECK(SendDlgItemMessageA(encodingWindow,77,14,0,0)==6);CHECK(SendDlgItemMessageW(encodingWindow,77,14,0,0)==6);{static char a[8];static WORD w[8];CHECK(SetDlgItemTextA(encodingWindow,77,"text"));CHECK(GetDlgItemTextW(encodingWindow,77,w,8)==4);CHECK(w[0]==116);CHECK(SetDlgItemTextW(encodingWindow,77,w));CHECK(GetDlgItemTextA(encodingWindow,77,a,3)==2);CHECK(a[0]==116&&a[1]==101&&!a[2]);CHECK(SetDlgItemInt(encodingWindow,77,0xffffffff,1));CHECK(GetDlgItemTextA(encodingWindow,77,a,8)==2);CHECK(a[0]==45&&a[1]==49&&!a[2]);{BOOL translated=0;CHECK(GetDlgItemInt(encodingWindow,77,&translated,1)==0xffffffff);CHECK(translated);}}SetWindowLongA(control,-4,(long)checkProc);CHECK(CheckDlgButton(encodingWindow,77,2));CHECK(IsDlgButtonChecked(encodingWindow,77)==2);CHECK(!SendDlgItemMessageW(encodingWindow,78,14,0,0));CHECK(GetLastError()==1421);CHECK(DestroyWindow(control));}
  {HWND check=CreateWindowExA(0,"BUTTON","Check",0x40000005,0,0,20,20,encodingWindow,(HANDLE)88,0,0);CHECK(check);CHECK(CheckDlgButton(encodingWindow,88,2));CHECK(IsDlgButtonChecked(encodingWindow,88)==2);CHECK(CheckDlgButton(encodingWindow,88,99));CHECK(IsDlgButtonChecked(encodingWindow,88)==2);CHECK(SendMessageA(check,0xf4,6,1)==0);CHECK((GetWindowLongA(check,-16)&15)==6);CHECK(SendMessageW(check,0x87,0,0)==0x2000);CHECK(SendMessageA(check,0xf5,0,0)==0);CHECK(IsDlgButtonChecked(encodingWindow,88)==0);CHECK(SendMessageW(check,0xf5,0,0)==0);CHECK(IsDlgButtonChecked(encodingWindow,88)==1);CHECK(CheckRadioButton(encodingWindow,88,88,99));CHECK(IsDlgButtonChecked(encodingWindow,88)==0);CHECK(CheckRadioButton(encodingWindow,88,88,88));CHECK(IsDlgButtonChecked(encodingWindow,88)==1);CHECK(DestroyWindow(check));}
  ...
  CHECK(!GetObjectType(encodingA.brush));

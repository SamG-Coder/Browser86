...
void mainCRTStartup(void){
  ...
  {HANDLE cursor=LoadCursorA(NULL,(const char*)32512);CHECK(cursor&&cursor==LoadCursorW(NULL,(const WORD*)32512));CHECK(!SetCursor(cursor));CHECK(GetCursor()==cursor);CHECK(SetClassLongW(encodingWindow,-12,(long)cursor)==0);CHECK(GetClassLongA(encodingWindow,-12)==(DWORD)cursor);CHECK(DestroyCursor(cursor));CHECK(SetCursor(NULL)==cursor);CHECK(SetClassLongA(encodingWindow,-12,0)==(DWORD)cursor);}

...
function run(){if(!process)return;try{process.tick(20000,10);}catch(e){process.fault(e);}flush();const now=performance.now();if(now-lastStats>500){postMessage({type:'stats',status:process.status,instructions:process.cpu.instructions,apiCalls:process.apiCalls,lastApi:process.lastApi,runtime:process.managed?'cil':'x86',committedBytes:process.memory.allocated+(process.managed?.allocatedBytes||0),waiting:process.waiting?.reason,elapsedMs:now-process.started});lastStats=now;}if(now-lastSave>3000){snapshot();lastSave=now;}if(['fault','exited','stopped'].includes(process.status)){snapshot(true);return;}loop=setTimeout(run,process.status==='paused'?60:process.waiting?12:0);}

...
import {installDoubleClick,classifyClick} from './double-click.js';
import {mouseMessage} from './mouse-messages.js';
...
export class GUI {
  ...
  install(){const a=this.api,p=this.p,m=this.m;const u=(name,n,fn,cdecl=false)=>a.add('user32.dll',name,n,fn,cdecl);const g=(name,n,fn)=>a.add('gdi32.dll',name,n,fn);
    ...
    for(const wide of [false,true]){const suffix=wide?'W':'A',str=pointer=>a.str(pointer,wide);
      ...
      u('GetClassName'+suffix,3,(h,out,n)=>{const w=this.window(h);if(!w)return a.fail(1400);n|=0;if(n<0)throw new RuntimeFault('UNSUPPORTED_GUI','Negative class-name buffer capacities are not implemented.');if(wide&&n<2)return a.fail(122);if(!n)return 0;const count=Math.min(w.className.length,n-1);checkBuffer(m,out,(count+1)*(wide?2:1));if(wide)m.string(out,w.className,true,count+1);else{for(let i=0;i<count;i++)m.w8(out+i,windowAnsiBestFit.get(w.className.charCodeAt(i))??63);m.w8(out+count,0);}return count;});

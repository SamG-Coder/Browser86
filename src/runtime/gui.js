...
export class GUI {
  ...
  install(){const a=this.api,p=this.p,m=this.m;const u=(name,n,fn,cdecl=false)=>a.add('user32.dll',name,n,fn,cdecl);const g=(name,n,fn)=>a.add('gdi32.dll',name,n,fn);
    for(const wide of [false,true]){const suffix=wide?'W':'A',str=pointer=>a.str(pointer,wide);
      for(const extended of [false,true])u('RegisterClass'+(extended?'Ex':'')+suffix,1,pointer=>{const o=extended?4:0;if(extended&&m.u32(pointer)!==48)return a.fail(87);checkBuffer(m,pointer,extended?48:40,'r');if(m.i32(pointer+o+8)<0||m.i32(pointer+o+12)<0)return a.fail(87);const proc=m.u32(pointer+o+4),name=str(m.u32(pointer+o+36));if(!name)return a.fail(87);if(this.classes.has(name.toLowerCase()))return a.fail(1410);const atom=this.nextAtom++;this.classes.set(name.toLowerCase(),{name,atom,proc,style:m.u32(pointer+o),classExtra:m.u32(pointer+o+8),windowExtra:m.u32(pointer+o+12),icon:m.u32(pointer+o+20),cursor:m.u32(pointer+o+24),menu:m.u32(pointer+o+32),smallIcon:extended?m.u32(pointer+44):0,instance:m.u32(pointer+o+16),background:m.u32(pointer+o+28),wide});return atom;});
      ...
      u('GetWindowLong'+suffix,2,(h,index)=>{const w=this.window(h);if(!w)return a.fail(1400);index|=0;if(index>=0)return this.windowExtra(w,index);return index===-4?w.proc:index===-12?w.id:index===-16?w.style:index===-20?w.exStyle:index===-21?w.userData||0:index===-8?w.parent:0;});
      u('SetWindowLong'+suffix,3,(h,index,value)=>{const w=this.window(h);if(!w)return a.fail(1400);index|=0;if(index>=0)return this.windowExtra(w,index,value);const field=index===-4?'proc':index===-12?'id':index===-16?'style':index===-20?'exStyle':index===-21?'userData':null;if(!field)return a.fail(87);const old=w[field]||0;w[field]=value;if(field==='proc')w.wide=wide;if(field==='style'){w.visible=!!(value&0x10000000);w.enabled=!(value&0x08000000);this.notify(w);}return old;});

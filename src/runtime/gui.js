...
export class GUI {
  ...
  install(){const a=this.api,p=this.p,m=this.m;const u=(name,n,fn,cdecl=false)=>a.add('user32.dll',name,n,fn,cdecl);const g=(name,n,fn)=>a.add('gdi32.dll',name,n,fn);
    for(const wide of [false,true]){const suffix=wide?'W':'A',str=pointer=>a.str(pointer,wide);
      ...
      u('GetWindowLong'+suffix,2,(h,index)=>{const w=this.window(h);if(!w)return a.fail(1400);index|=0;return index===-4?w.proc:index===-12?w.id:index===-16?w.style:index===-20?w.exStyle:index===-21?w.userData||0:index===-8?w.parent:0;});
      u('SetWindowLong'+suffix,3,(h,index,value)=>{const w=this.window(h);if(!w)return a.fail(1400);index|=0;const field=index===-4?'proc':index===-12?'id':index===-16?'style':index===-20?'exStyle':index===-21?'userData':null;if(!field)return a.fail(87);const old=w[field]||0;w[field]=value;if(field==='style'){w.visible=!!(value&0x10000000);w.enabled=!(value&0x08000000);this.notify(w);}return old;});
    ...
    u('SetCursor',1,cursor=>cursor);u('GetSystemMetrics',1,index=>({0:1280,1:720,2:17,3:17,4:28,5:1,6:1,32:4,33:4,61:1,80:1}[index]??0));installWindowProperties(this);installWindowFocus(this);installWindowState(this);installWindowHierarchy(this);installWindowCoordinates(this);installSystemColors(this);installRegions(this);

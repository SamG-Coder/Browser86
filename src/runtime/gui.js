...
export class GUI {
  ...
  install(){const a=this.api,p=this.p,m=this.m;const u=(name,n,fn,cdecl=false)=>a.add('user32.dll',name,n,fn,cdecl);const g=(name,n,fn)=>a.add('gdi32.dll',name,n,fn);
    for(const wide of [false,true]){const suffix=wide?'W':'A',str=pointer=>a.str(pointer,wide);
      ...
      u('SetWindowLong'+suffix,3,(h,index,value)=>{const w=this.window(h);if(!w)return a.fail(1400);index|=0;const field=index===-4?'proc':index===-12?'id':index===-16?'style':index===-20?'exStyle':index===-21?'userData':null;if(!field)return a.fail(87);const old=w[field]||0;w[field]=value;return old;});
    ...
    u('ShowWindow',2,(h,command)=>{const w=this.window(h);if(!w)return 0;const old=w.visible;w.visible=command!==0;this.notify(w);if(w.visible){this.queuePaint(w);p.postMessage(h,WM_SIZE,0,(w.width|(w.height<<16))>>>0);}return old?1:0;});

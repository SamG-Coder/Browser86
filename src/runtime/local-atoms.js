...
export function installLocalAtoms(api){
 ...
 const read=(pointer,wide,add)=>{
  ...
  if(!ended)return {error:add?87:2};if(!units.length)return {error:123};
 ...
 for(const wide of [false,true]){
  ...
  k('GetAtomName'+(wide?'W':'A'),3,(id,out,size)=>{
   ...
   const text=id<0xc000?'#'+id:atoms.get(id)?.text;if(text===undefined)return api.fail(6);if(size===1)return api.fail(122);

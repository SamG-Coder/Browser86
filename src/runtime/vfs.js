...
export class VirtualFileSystem {
  ...
  mkdir(p){const path=this.path(p);const parts=path.slice(3).split('/').filter(Boolean);let current='C:/';
    ...
    for(const x of parts){current+=(current.endsWith('/')?'':'/')+x;const key=current.toLowerCase(),old=this.nodes.get(key);requireThat(!old?.deletePending,'VFS_DELETE_PENDING','Directory is pending deletion.');requireThat(!old||old.directory,'VFS_NOT_DIRECTORY','A parent path is a file.',{path:current});if(!old){this.nodes.set(key,this.newNode(current,true));this.revision++;}}
    return true;
  }
  ...
  rename(from,to){const src=this.get(from),path=this.path(to),key=this.key(to);requireThat(src,'VFS_NOT_FOUND','Source file does not exist.');requireThat(!src.deletePending&&!this.get(to)?.deletePending,'VFS_DELETE_PENDING','File is pending deletion.');requireThat(this.key(from)!=='c:/','VFS_OPERATION','The virtual drive root cannot be renamed.');requireThat(!src.directory||!key.startsWith(this.key(from)+'/'),'VFS_OPERATION','Cannot move a directory into itself.');requireThat(!this.exists(to)||this.key(from)===key,'VFS_EXISTS','Destination exists.');this.mkdir(path.slice(0,path.lastIndexOf('/'))||'C:/');const oldKey=this.key(from),oldPath=src.path,moved=[...this.nodes.values()].filter(n=>n===src||src.directory&&n.path.toLowerCase().startsWith(oldKey+'/')).map(n=>({node:n,path:path+n.path.slice(oldPath.length)}));for(const item of moved)requireThat(item.path.length<=32767,'VFS_PATH','Renamed descendant path is too long.');for(const item of moved)this.nodes.delete(item.node.path.toLowerCase());for(const item of moved){item.node.path=item.path;this.nodes.set(item.path.toLowerCase(),item.node);}this.revision++;return true;}

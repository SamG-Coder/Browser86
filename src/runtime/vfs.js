...
export class VirtualFileSystem {
  ...
  writeFile(p,bytes,{preserveWriteTime=false,preserveAccessTime=true}={}){const path=this.path(p);const data=bytes instanceof Uint8Array?bytes:new Uint8Array(bytes);const old=this.get(path);requireThat(!old?.directory,'VFS_IS_DIRECTORY','Cannot write a directory.',{path});const newSize=this.bytes-(old?.data.length||0)+data.length;requireThat(newSize<=this.limit,'VFS_QUOTA','Virtual disk quota exceeded.');this.mkdir(path.slice(0,path.lastIndexOf('/'))||'C:/');const node=old||this.newNode(path,false);node.data=data.slice();node.attributes=(node.attributes&~128)|32;
  ...
  snapshot(){return [...this.nodes.values()].map(n=>({path:n.path,directory:n.directory,data:n.data?.slice(),mtime:n.mtime,...fileMetadata(n)}));}

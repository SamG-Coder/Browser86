...
export class CPU {
  ...
  extended(w){const op=this.next8();
    ...
    if(op===0xfe&&this.op16&&!this.rep){const o=this.operand(32),bytes=o.isReg?this.xmm[o.rm].slice():this.m.read(o.a,16),source=new DataView(bytes.buffer,bytes.byteOffset,16),dest=new DataView(this.xmm[o.reg].buffer);for(let i=0;i<16;i+=4)dest.setUint32(i,dest.getUint32(i,true)+source.getUint32(i,true),true);return;}
    if(op===0x5b&&!this.op16&&!this.rep){const o=this.operand(32),bytes=o.isReg?this.xmm[o.rm].slice():this.m.read(o.a,16),source=new DataView(bytes.buffer,bytes.byteOffset,16),dest=new DataView(this.xmm[o.reg].buffer);for(let i=0;i<16;i+=4)dest.setFloat32(i,source.getInt32(i,true),true);return;}
  ...
  sse(op){
    ...
    if(op===0x7E){if(this.rep===0xF3){const data=load(8);dst.fill(0);dst.set(data);}else if(this.op16)o.set(new DataView(dst.buffer).getUint32(0,true));else this.unsupported('MMX is not implemented.');return;}
    ...
    if(op===0x70){if(!this.op16&&!this.rep)this.unsupported('MMX shuffle is not implemented.');const imm=this.next8(),data=load(16);if(this.rep){const base=this.rep===0xf3?8:0;dst.set(data);for(let i=0;i<4;i++){const source=base+((imm>>>(i*2))&3)*2;dst.set(data.subarray(source,source+2),base+i*2);}}else for(let i=0;i<4;i++)dst.set(data.subarray(((imm>>>(i*2))&3)*4,(((imm>>>(i*2))&3)+1)*4),i*4);return;}
    ...
    if(op===0x5A){if(!scalar){const data=load(double?16:8),source=new DataView(data.buffer),a=double?source.getFloat64(0,true):source.getFloat32(0,true),b=double?source.getFloat64(8,true):source.getFloat32(4,true);if(double){dst.fill(0);dv.setFloat32(0,a,true);dv.setFloat32(4,b,true);}else{dv.setFloat64(0,a,true);dv.setFloat64(8,b,true);}return;}const data=load(width),v=readFloat(new DataView(data.buffer),0);if(double)dv.setFloat32(0,v,true);else dv.setFloat64(0,v,true);return;}

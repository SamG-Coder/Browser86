...
export class GuestDisplay {
  ...
  window({op,window:w}){
    ...
    if(!record){
      if((w.style&0x40000000)&&w.parent&&this.windows.has(w.parent)){const parent=this.windows.get(w.parent),type=w.className.toUpperCase(),node=element('canvas','guest-control');node.tabIndex=0;node.addEventListener('click',()=>{if(node.getAttribute('aria-disabled')==='true')return;node.focus();if(type==='BUTTON')this.input({kind:'click',hwnd:w.hwnd});});for(const [dom,down]of [['keydown',true],['keyup',false]])node.addEventListener(dom,e=>{e.preventDefault();if(node.getAttribute('aria-disabled')==='true')return;this.input({kind:'key',hwnd:w.hwnd,down,code:e.keyCode,char:e.key.length===1?e.key:'',ctrlKey:e.ctrlKey,shiftKey:e.shiftKey,altKey:e.altKey});});(parent.client||parent.element).append(node);record={element:node,canvas:node,context:node.getContext('2d'),control:true,type};}
        ...
        for(const [dom,down]of [['keydown',true],['keyup',false]])canvas.addEventListener(dom,e=>{if(['F5','F11','F12'].includes(e.key)||e.ctrlKey&&['r','l'].includes(e.key.toLowerCase()))return;e.preventDefault();this.input({kind:'key',hwnd:w.hwnd,down,code:e.keyCode,ctrlKey:e.ctrlKey,shiftKey:e.shiftKey,altKey:e.altKey,char:e.key.length===1?e.key:e.key==='Enter'?'\r':e.key==='Backspace'?'\b':''});});
      }
  ...
  draw(commands){for(const c of commands){const record=this.windows.get(c.hwnd);if(!record?.context)continue;const ctx=record.context;ctx.save();try{
      ...
      if(c.gdipClip)for(const polygon of c.gdipClip){ctx.beginPath();polygon.forEach(([x,y],i)=>i?ctx.lineTo(x,y):ctx.moveTo(x,y));ctx.closePath();ctx.clip();}
      ...
      if(c.transform)ctx.transform(...c.transform);

...
export class GuestDisplay {
  ...
  window({op,window:w}){
    ...
    if(!record){
      if(w.parent&&this.windows.has(w.parent)){const parent=this.windows.get(w.parent),type=w.className.toUpperCase(),node=element(type==='BUTTON'?'button':type==='EDIT'?'input':'div','guest-control '+(type==='BUTTON'?'button':type==='EDIT'?'edit':'static'));if(type==='BUTTON')node.addEventListener('click',()=>this.input({kind:'click',hwnd:w.hwnd}));if(type==='EDIT')node.addEventListener('input',()=>this.input({kind:'edit',hwnd:w.hwnd,text:node.value}));parent.client.append(node);record={element:node,control:true,type};}
  ...
  draw(commands){for(const c of commands){const record=this.windows.get(c.hwnd);if(!record?.context)continue;const ctx=record.context;ctx.save();try{
      ...
      else if(c.op==='polygon'||c.op==='polyline'){
        ctx.beginPath();c.points.forEach(([x,y],i)=>i?ctx.lineTo(x,y):ctx.moveTo(x,y));
        if(c.op==='polygon'){ctx.closePath();if(c.brush&&!c.brush.null){ctx.fillStyle=color(c.brush.color);ctx.fill(c.fillMode===2?'nonzero':'evenodd');}}

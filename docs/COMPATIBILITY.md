...
Snapshots, IndexedDB and exported backup ZIPs preserve file IDs, attributes and exact timestamps. Old backups without metadata remain readable. IDs and the reported volume serial identify this virtual disk, not host NTFS objects. Hard links, full delete-pending/open-file lifetime, NTFS security descriptors and GetFileInformationByHandleEx classes remain separate compatibility gaps.

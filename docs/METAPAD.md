...
Registered messages and menu-item updates are now implemented. `RegisterWindowMessageA/W` uses case-insensitive names shared between the ANSI/Unicode entrypoints for the isolated guest session. Native probes established null/empty/overlong-name behavior and integer-name handling. Cross-process system-wide registration is not claimed. `SetMenuItemInfoA/W` supports state, string/separator type, command ID, item data, and submenu updates with cycle validation; bitmap/owner-drawn modes remain explicit unsupported paths.

The latest browser probe stops at `comctl32.dll!CreateToolbarEx`, caller `0x0040215f`, with 53 unresolved imports and no browser JavaScript errors. Toolbar creation, rendering, and input are the next required control implementation. The editor acceptance baseline remains incomplete. Validation: 562 Node tests and the four GDI/control browser checks pass.

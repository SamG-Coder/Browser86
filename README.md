...
`src/api-catalog.json` contains **565 registered import entries**, including A/W variants, CRT functions and data entries. **That count is not 565 fully compatible Windows APIs.** Many handlers implement a restricted argument/behavior subset; others explicitly reject unsupported modes. See the source and compatibility notes before assuming parity.

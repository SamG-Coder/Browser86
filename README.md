...
`src/api-catalog.json` contains **599 registered import entries**, including A/W variants, CRT functions and data entries. **That count is not 599 fully compatible Windows APIs.** Many handlers implement a restricted argument/behavior subset; others explicitly reject unsupported modes. See the source and compatibility notes before assuming parity.

...
try{
 ...
 const expected=[255,0,255,255,0];results.forEach((pixel,i)=>{if(pixel[0]!==expected[i]||pixel[1]!==expected[i]||pixel[2]!==expected[i])throw Error('Polygon canvas case '+i+' failed: '+pixel);});
 const report={result:'PASS',browser:browser.version(),checks:['alternate leaves double-wound interior empty','winding fills double-wound interior','raw mode 3 uses alternate coverage','polyline remains open','polygon closes its outline'],pixels:results};
 fs.writeFileSync('docs/test-artifacts/polygons-canvas-report.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));

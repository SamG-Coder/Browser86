...
def main():
    ...
    with sync_playwright() as pw:
        ...
        page.evaluate("(()=>{const dt=new DataTransfer();dt.items.add(new File([__testZip],'browser86-demo.zip',{type:'application/zip'}));window.dispatchEvent(new DragEvent('drop',{dataTransfer:dt,cancelable:true}));})()");page.wait_for_timeout(1200);print('Import status:',page.locator('#toast').text_content(),page.locator('#progress-text').text_content());page.wait_for_function("document.querySelectorAll('.exe-option').length===7",timeout=20000)
        report['assertions'].append('Drag/drop flow and original import worker discover seven EXEs from DEFLATE ZIP')

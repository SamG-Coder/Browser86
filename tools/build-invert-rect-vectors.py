...
try:
    for color in [0,0xffffff,0x123456,0xff0080,0x808080]:
        ...
        try:
            U.FillRect(dc,(c.c_int*4)(0,0,8,8),brush);assert U.InvertRect(dc,(c.c_int*4)(7,7,1,1))
            pixels=[G.GetPixel(dc,x,y) for y in range(8) for x in range(8)]
            assert U.InvertRect(dc,(c.c_int*4)(7,7,1,1))
...
Path('tests/invert-rect-vectors.json').write_text(json.dumps(results,separators=(',',':'))+'\n')
print('Recorded five native reversed-rectangle RGB inversion/restore cases; owned resources released.')

...
call('SelectClipRgn',dc,clip);call('SelectObject',dc,call('GetStockObject',4));
...
try{const page=await browser.newPage();await page.goto(process.argv[2]);const results=await page.evaluate(async cases=>{
 ...
 const report={result:'PASS',browser:browser.version(),checks:cases.map(v=>v.name)};fs.writeFileSync('docs/test-artifacts/clip-region-canvas-report.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));

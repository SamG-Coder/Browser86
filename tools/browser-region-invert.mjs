...
const vectors=JSON.parse(fs.readFileSync('tests/invert-region-vectors.json','utf8'));
...
call('InvertRgn',dc,r);const command=events.filter(e=>e.type==='draw').at(-1);
...
try{
 ...
 const report={result:'PASS',browser:browser.version(),checks:15,description:'Five colors: all 64 pixels match native inversion and restoration, and subsequent fills retain normal compositing.',results};fs.writeFileSync('docs/test-artifacts/invert-region-canvas-report.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({result:report.result,browser:report.browser,checks:report.checks}));
